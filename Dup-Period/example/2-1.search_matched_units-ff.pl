...
open FILE, "gzip -dc $dir1\/g\-${sample}\.fas\.gz | " or die "Can not open seq1 file\n";
...
open FILE, "gzip -dc $dir2\/g\-$sample\.fas.gz | " or die "Can not open seq2 file\n";
open FILEOUT, " | gzip > $dirout\/gp\-$sample\.txt\.gz" or die "Can not open seq out file\n";
while (<FILE>) { s/\r//g; chomp;
	if (/>(.+)/) {
